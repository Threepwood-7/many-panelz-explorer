...
def _windows_dir(env_name: str, fallback_parts: tuple[str, ...]) -> Path:
...
def resolve_config_root(override_dir: str | None = None) -> Path:
    ...
    if override:
        ...
        root = _windows_dir("APPDATA", ("AppData", "Roaming"))
...
def resolve_data_root(override_dir: str | None = None) -> Path:
    ...
    if override:
        ...
        root = _windows_dir("LOCALAPPDATA", ("AppData", "Local")) / SETTINGS_ORG_NAME
    root.mkdir(parents=True, exist_ok=True)
    return root
